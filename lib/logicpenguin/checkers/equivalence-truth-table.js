...
export default async function(
    ...
    if (opts.highlightWitnessRow) {
        const isValidWitness = (i) => answerTables.every(
            (table) => table.rows[i]?.[table.opspot] === true
        );
    ...
    if (opts.question) { componentScores.push(mcScore); }
    if (opts.highlightWitnessRow) { componentScores.push(witnessScore); }
