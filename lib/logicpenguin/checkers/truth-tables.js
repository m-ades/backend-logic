...
// checks that exactly one row is highlighted, and that it is a valid
// witness according to the caller-supplied predicate; used for the
// "highlight a row that witnesses X" graded option shared by all three
// truth-table checkers
export function hasSingleRowHighlight(givenans, isValidWitness) {
    ...
    const selected = highlights.reduce((acc, v, i) => { if (v) acc.push(i); return acc; }, []);
    return selected.length === 1 && isValidWitness(selected[0]);
}
