...
export default class DerivationCheck {
    ...
    checkCP(line) {
        if (!line?.formulaNormal || line.formulaNormal.indexOf("⊃") === -1) {
            this.adderror(line.n, "rule", "high",
                "CP requires a conditional conclusion (e.g. A ⊃ B).");
            return line;
        }
        ...
        if (assumptionFormula !== antecedent) {
            this.adderror(line.n, "rule", "high",
                "CP requires the antecedent to match the assumption (ACP).");
            return line;
        }
    ...
    checkIP(line) {
        ...
        const assumptionFormula = assumptionLine.formulaNormal ??
            (assumptionLine?.s ? Formula.from(assumptionLine.s).normal : '');
        let expected = "~" + (assumptionFormula ?? '').trim();
        if (assumptionFormula) {
            try {
                expected = Formula.from(`~(${assumptionFormula})`).normal;
            } catch {
                // fall back to simple string negation
            }
        }
