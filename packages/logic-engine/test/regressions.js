...
export const regressions = [
  ['composite multiple choice accepts both field names and submission shapes', async () => {
    ...
    for (const field of ['questions', 'subquestions']) {
      for (const submission of [{ answers: [1, 0, [2, 0]] }, { ans: [1, 0, [2, 0]] }, [1, 0, [2, 0]]]) {
        const result = await checkers['multiple-choice']({ [field]: subquestions }, null, submission, true, 100, false, {});
        assert.equal(result.successstatus, 'correct');
        assert.equal(result.points, 100);
      }
      const missing = await checkers['multiple-choice']({ [field]: [{ answerIndex: 0 }] }, null, { answers: [''] }, true, 100, false, {});
      assert.equal(missing.points, 0);
    }
    const embedded = await checkers['multiple-choice']({ answerIndex: 1 }, undefined, { ans: 1 }, false, 100, false, {});
    assert.equal(embedded.points, 100);
  }],
  ['combo grading preserves table and translation weights and rejects missing tables', async () => {
    ...
    for (const partial of [false, true]) {
      const missing = await checker({}, answer, { argumentLine: 'A // B' }, partial, 60, false, { notation: 'hurley' });
      assert.equal(missing.successstatus, partial ? 'partial' : 'incorrect');
      assert.equal(missing.points, partial ? 20 : 0);
      ...
      }, partial, 60, false, { notation: 'hurley' });
      assert.equal(translated.successstatus, partial ? 'partial' : 'incorrect');
      assert.equal(translated.points, partial ? 40 : 0);
    }
    const absent = await checker({}, answer, undefined, false, 60, false, { notation: 'hurley' });
    ...
    for (const [kind, statements, witness] of [
      ...
      for (const partial of [false, true]) {
        for (const rowhls of [[], tables[0].rows.map(() => true), tables[0].rows.map((_, index) => index === tables[0].rows.length - 1)]) {
          const result = await checker(question, answer, { lefts: tables.slice(0, -1), right: tables.at(-1), rowhls }, partial, 100, false, options);
          assert.equal(result.successstatus, partial ? 'partial' : 'incorrect');
          assert.equal(result.points, partial ? 50 : 0);
        }
      }
      ...
      }, false, 100, false, options);
      assert.equal(correct.points, 100);
    }
  }],
  ['main operator highlights and normalized truth values affect grading', async () => {
    ...
    for (const [colhls, points] of [[[false, true, false], 100], [[true, false, false], 50], [[], 50]]) {
      const result = await checkers['formula-truth-table'](question, answer, { right: { rows, colhls } }, true, 100, false, { highlightMainOperator: true });
  ...
  ['symbol picker presentation selectors are ignored by parsing and translation', async () => {
    for (const notation of ['calgary', 'hurley']) {
      ...
      for (const selector of ['\uFE0E', '\uFE0F']) {
        ...
        const result = await checkers['symbolic-translation']({}, 'A ↔ B', `A ↔${selector} B`, false, 100, false, { notation, pred: false });
        assert.equal(result.points, 100);
      }
    }
  }],
  ['missing translations and malformed hurley subderivations earn no credit', async () => {
    for (const input of [null, undefined, '', ' ', 7, {}]) {
      const result = await checkers['symbolic-translation']({}, 'A', input, false, 100, false, { notation: 'hurley', pred: false });
      assert.equal(result.points, 0);
    }
    for (const proof of [null, undefined, {}, { parts: null }, { parts: {} }, { parts: [{ parts: null }] }]) {
      const result = await checkers['derivation-hurley']({ prems: ['A'], conc: 'A' }, { parts: [{ n: '1', s: 'A', j: 'Pr' }] }, proof, false, 100, false, {});
